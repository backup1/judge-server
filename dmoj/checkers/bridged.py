...
def check(process_output, judge_output, judge_input, problem_id,
    ...
    if returncode not in contrib_modules or not hasattr(contrib_modules[returncode].ContribModule, 'parse_return_code'):
        raise InternalError('%s is not a valid return code parser' % returncode)
    returncode_parser = contrib_modules[returncode].ContribModule.parse_return_code
